...
static HOT_WORDS: LazyLock<RwLock<Arc<Vec<String>>>> =
...
pub(crate) fn get_hot_words(count: usize) -> Vec<String> {
    ...
    hot_words.iter().take(count).cloned().collect()
}
...
pub(crate) fn fetch_hot_words() -> anyhow::Result<()> {
    ...
    all_hot_words.sort();
    all_hot_words.dedup();
    all_hot_words.shuffle(&mut rand::rng());
    let mut hot_words_lock = HOT_WORDS.write().unwrap();
    *hot_words_lock = Arc::new(all_hot_words);
    Ok(())
}
